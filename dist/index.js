...
const client = new discord_js_1.default.Client({ intents: env.bot.intents });
...
client.on("messageCreate", (message) => {
    ...
    if (message.author.bot)
        return;
    level_1.default.give(message);
    if (!message.content.startsWith(env.bot.prefix))
        return;
    ...
    if (command === "level")
        level_1.default.check(message, args);
    return;
});
